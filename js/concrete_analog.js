...
function concrete_analog(){
    ...
    function forward(tracks) {
        ...
        container = document.getElementById( 'ConcreteAnalog' );
        ...
        //camera = new THREE.PerspectiveCamera( -10,window.innerWidth / window.innerHeight, -10, 10 );
        w=422.;
        h=12;
        camera = new THREE.OrthographicCamera(-w/2.0, w/2.0, h/2.0, -h/2.0, -h/2, h/2);
        //camera.position.z = 30;
        //camera.position.y = 0;
        camera.position.set(211,0,5);
        //camera.lookAt(211,0,0);
        scene = new THREE.Scene();

        //var geometry = new THREE.BoxGeometry(100, 100, 10, 1, 1, 1);
        var geometry = new THREE.BoxGeometry(422, 12, 12);
    ...
    //        return 0;
        ...
        var linematerial = new THREE.LineBasicMaterial( { color: 'rgba(0,102,255,1)', linewidth:3, transparent:false} );
        ...
        function plotBypass(){
            var lineb= new THREE.LineBasicMaterial( { color: 'rgba(0,0,0,1)', linewidth:10, transparent:false} );
            var geo_left= new THREE.Geometry();
            geo_left.vertices.push(new THREE.Vector3(0,5,0));
            geo_left.vertices.push(new THREE.Vector3(0,-5,0));
            ln_left= new THREE.Line( geo_left,  lineb);

            var geo_right= new THREE.Geometry();
            geo_right.vertices.push(new THREE.Vector3(420,5,0));
            geo_right.vertices.push(new THREE.Vector3(420,-5,0));
            ln_right= new THREE.Line( geo_right,  lineb);

            var geo_top= new THREE.Geometry();
            geo_top.vertices.push(new THREE.Vector3(0,5,0));
            geo_top.vertices.push(new THREE.Vector3(420,5,0));
            ln_top= new THREE.Line( geo_top,  lineb);

            var geo_bottom= new THREE.Geometry();
            geo_bottom.vertices.push(new THREE.Vector3(0,-5.0,0));
            geo_bottom.vertices.push(new THREE.Vector3(420,-5.0,0));
            ln_bottom= new THREE.Line( geo_bottom, lineb);
        ...
        function addEye(){
            ...
            var matEye= new THREE.MeshLambertMaterial({
                map: loader.load('figures/concrete_geom.png')
            });

            //create a plane geometry for the image with a width of 10
            // and a height that preserves the image's aspect ratio
            var geoEye= new THREE.PlaneGeometry(840, 80);
            ...
            meshEye.position.set(220.,0.,0);
            //meshEye.rotateZ(Math.PI+Math.PI/7.);
            //meshEye.scale.set(0.8,0.8,1);
...
//                .start();
    ...
    function readCSV(tracks) {
        ...
        jQuery.get('tracks/concrete_analog.tracks', function(data) {
        //jQuery.get('tracks/concrete_et.tracks', function(data) {
